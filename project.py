...
df=pd.read_excel(r'C:\Users\aksha\Desktop\PYTHON\Classes\Project\Dataset.xlsx',skiprows=7)
headers=df.iloc[0]
global df1
...
def adjustWindow(window):
 ...
 window.iconbitmap(r'C:\Users\aksha\Desktop\PYTHON\Classes\Project\icon.ico')
...
def viewpoint():
    ...
    width = .5
    ax1.bar(ca_agents.index,ca_agents['Area'])
    ax2.bar(ws_agents.index,ws_agents['Area'])
    ...
    Label(screen30, text='AS THERE ARE ONLY 4 AGENTS WORKING IN WS COUNTRY THE BUSINESS IS MUCH MORE LESS AS COMPARED TO CA.SO IF MORE AGENTS WORK IN WS LIKE IN CA THE BUSINESS IN',font=("Open Sans", 12, 'bold'),fg='black', bg='white').place(x=10,y=10)
    Label(screen30, text='WS CAN IMPROVE',font=("Open Sans", 12, 'bold'),fg='black', bg='white').place(x=10,y=40)
 
   
def get_customer_code():
    connection = pymysql.connect(host="localhost", user="root",passwd="", database="project") # database connection
        ...
        for i in range(len(orders)):
            if cust_code in orders[i][4]:
                if order_num == orders[i][0]:
                    ...
                    for j in range(7):
                        Label(screen34,text=orders[i][j],font=("Calibri", 14, 'bold'),fg='black',bg="#dfe6e9").grid(row=2+i,column=j)
...
def module2():
    ...
    connection = pymysql.connect(host="localhost", user="root",passwd="", database="project") # database connection
    cursor = connection.cursor()
    select_query = "SELECT CUST_CODE FROM customer  "
    select_query1='SELECT ORD_NUM FROM orders'
   ...
   # Entry(screen33,textvariable=order_number_verify).pack() 
    Combobox(screen33, width=9, values = order_numbers ,textvariable = order_number_verify).pack()
    Label(screen33,text='',bg="#dfe6e9").pack()
    order_number_verify.set("")
    Label(screen33, text="Enter the order date",font=("Roboto", 15, 'bold'), fg='black',bg="#dfe6e9").pack()
    year= StringVar() 
    choices= list(range(2005,2021))
    Label(screen33,text='',bg="#dfe6e9").pack()
    ...
    Combobox(screen33, width=9, values = customer_codes ,textvariable = customer_code_verify).pack()
...
#    q7ad.plot(ax=ax,x='Year')
    ...
    #ax.set_title('Time series for Area leased and Owned')
...
def piechart_2017():
    ...
    labels = 'leased', 'Owned'
    leased_df=df1[(df1['Tenure']=='Leased') & (df1['Year']==2017)]
    owned_df=df1[(df1['Tenure']=='Owned') & (df1['Year']==2017)]
    leased=sum(leased_df['Area'])
    owned=sum(owned_df['Area'])
    sizes=[leased,owned]
    ...
    Label(screen31,text="Owned Area : ",font=("Open Sans", 15, 'bold'),bg="white").place(x=10,y=30)
    Label(screen31,text="379589.8",font=("Open Sans", 15, 'bold'),bg="white").place(x=170,y=30)
    Label(screen31,text="Leased Area: ",font=("Open Sans", 15, 'bold'),bg="white").place(x=350,y=30)
    Label(screen31,text="145403.87",font=("Open Sans", 15, 'bold'),bg="white").place(x=510,y=30)
...
def piechart_2018():
    ...
    adjustWindow(screen31)
    labels = 'leased', 'Owned'
    leased_df=df1[(df1['Tenure']=='Leased') & (df1['Year']==2018)]
    owned_df=df1[(df1['Tenure']=='Owned') & (df1['Year']==2018)]
    leased=sum(leased_df['Area'])
    owned=sum(owned_df['Area'])
    sizes=[leased,owned]   
    f = Figure(figsize=(5,5), dpi=120)
    ...
    Button(screen31, text='Back->',width=13, font=("Open Sans", 15, 'bold'),fg='black', bg='#84DCC6',command=screen31.destroy).place(x=700,y=700)
    Label(screen31,text="Owned Area : ",font=("Open Sans", 15, 'bold'),bg="white").place(x=10,y=30)
    Label(screen31,text="486892.37",font=("Open Sans", 15, 'bold'),bg="white").place(x=170,y=30)
    Label(screen31,text="Leased Area: ",font=("Open Sans", 15, 'bold'),bg="white").place(x=350,y=30)
    Label(screen31,text="272277.96",font=("Open Sans", 15, 'bold'),bg="white").place(x=510,y=30)
...
def piechart_2019():
    ...
    adjustWindow(screen31)
    labels = 'leased', 'Owned'
    leased_df=df1[(df1['Tenure']=='Leased') & (df1['Year']==2019)]
    owned_df=df1[(df1['Tenure']=='Owned') & (df1['Year']==2019)]
    leased=sum(leased_df['Area'])
    owned=sum(owned_df['Area'])
    sizes=[leased,owned]
    f = Figure(figsize=(5,5), dpi=120)
    ...
    Button(screen31, text='Back->',width=13, font=("Open Sans", 15, 'bold'),fg='black', bg='#84DCC6',command=screen31.destroy).place(x=700,y=700)
    Label(screen31,text="Owned Area : ",font=("Open Sans", 15, 'bold'),bg="white").place(x=10,y=30)
    Label(screen31,text="4348387.99",font=("Open Sans", 15, 'bold'),bg="white").place(x=170,y=30)
    Label(screen31,text="Leased Area: ",font=("Open Sans", 15, 'bold'),bg="white").place(x=350,y=30)
    Label(screen31,text="909204",font=("Open Sans", 15, 'bold'),bg="white").place(x=510,y=30)
...
def piechart_2020():
    ...
    #adjustWindow(screen31)
    labels = 'leased', 'Owned'
    leased_df=df1[(df1['Tenure']=='Leased') & (df1['Year']==2020)]
    owned_df=df1[(df1['Tenure']=='Owned') & (df1['Year']==2020)]
    leased=sum(leased_df['Area'])
    owned=sum(owned_df['Area'])
    sizes=[leased,owned]
    f = Figure(figsize=(5,5), dpi=120)
    ...
    ax1.pie(sizes, explode=explode, labels=labels, autopct='%1.1f%%',shadow=True, startangle=90)
    ax1.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    canvas = FigureCanvasTkAgg(f, master=screen18)
    canvas.get_tk_widget().pack(side=sk.TOP, fill=sk.BOTH, expand=1)
    Button(screen18, text='Back->',width=13, font=("Open Sans", 15, 'bold'),fg='black', bg='#84DCC6',command=screen18.destroy).place(x=700,y=700)
    Label(screen18,text="Owned Area : ",font=("Open Sans", 15, 'bold'),bg="white").place(x=10,y=30)
    Label(screen18,text="414104",font=("Open Sans", 15, 'bold'),bg="white").place(x=170,y=30)
    Label(screen18,text="Leased Area: ",font=("Open Sans", 15, 'bold'),bg="white").place(x=350,y=30)
    Label(screen18,text="118124",font=("Open Sans", 15, 'bold'),bg="white").place(x=510,y=30)
   
def ans_a():
    global screen18
    screen18 = Toplevel(screen)
    screen18.title("SELECT YEAR")
    adjustWindow(screen18)
    Button(screen18, text='2017',width=15, font=("Open Sans", 15, 'bold'),fg='black', bg='#E0ACD5',command=piechart_2017).place(x=200,y=50)
    Button(screen18, text='2018',width=15, font=("Open Sans", 15, 'bold'),fg='black', bg='#E0ACD5',command=piechart_2018).place(x=400,y=50)
    Button(screen18, text='2019',width=15, font=("Open Sans", 15, 'bold'),fg='black', bg='#E0ACD5',command=piechart_2019).place(x=600,y=50)
    Button(screen18, text='2020',width=15, font=("Open Sans", 15, 'bold'),fg='black', bg='#E0ACD5',command=piechart_2020).place(x=800,y=50)
...
def ans_b():
    ...
    screen26.title("MAXIMUM LEASED AREA IN CA AND WS COUNTRIES")
    ...
    #canvas.show()
    canvas.get_tk_widget().pack(side=sk.TOP, fill=sk.BOTH, expand=1)
    Label(screen26, text='2019 GOT MAXIMUM LEASED AREA IN CA AND WS COUNTRIES',font=("Roboto", 15, 'bold'),fg='black',bg="white").place(x=500,y=10)
...
def ans_d():
    ...
    width = .5
    rects1 = ax.bar(ans_d.idxmax(), ans_d.values, width)
    ...
    Label(screen29, text='got highest number of deals in Chilliwack',font=("Open Sans", 15, 'bold'),fg='black', bg="white").place(x=560,y=10)
...
def ans_e():
    ...
    #canvas.show()
    canvas.get_tk_widget().pack(side=sk.TOP, fill=sk.BOTH, expand=1)
    Label(screen27, text='MUKESH IS THE BEST PERFORMER AS HE IS CONSISTENT IN BOTH THE CATEGORIES',font=("Open Sans", 12, 'bold'),fg='black', bg="white").place(x=10,y=10)
...
def ans_f():
    ...
    Label(screen28, text=ans.index[0],font=("Open Sans", 15, 'bold'),fg='black', bg="white").place(x=400,y=10)
    Label(screen28, text=val[0],font=("Open Sans", 15, 'bold'),fg='black', bg="white").place(x=460,y=10)
    Label(screen28, text=ans.index[1],font=("Open Sans", 15, 'bold'),fg='black', bg="white").place(x=590,y=10)
    Label(screen28, text=val[1],font=("Open Sans", 15, 'bold'),fg='black', bg="white").place(x=650,y=10)
    Label(screen28, text=ans.index[2],font=("Open Sans", 15, 'bold'),fg='black', bg="white").place(x=790,y=10)
    Label(screen28, text=val[2],font=("Open Sans", 15, 'bold'),fg='black', bg="white").place(x=850,y=10)
    Label(screen28, text=ans.index[3],font=("Open Sans", 15, 'bold'),fg='black', bg="white").place(x=1010,y=10)
    Label(screen28, text=val[3],font=("Open Sans", 15, 'bold'),fg='black', bg="white").place(x=1075,y=10)
...
def company_insights():
    ...
    Button(screen8, text='YEAR WHICH GOT MAXIMUM LEASED AREA IN CA AND WS COUNTRIES', width=120, font=("Roboto", 13, 'bold'),bg='#E0ACD5', fg='black',command=ans_b).pack()
    Label(screen8, text="",bg="#dfe6e9").pack()
    ...
    Label(screen8, text="",bg="#dfe6e9").pack()
    Button(screen8, text='Back ->', width=12, font=("Open Sans", 15,'bold'), bg='#84DCC6', fg='black',command=screen8.destroy).place(x=670, y=720) # button to navigate back to login page
                     ...
                     if(re.match('^[a-zA-Z]+(?:[\s-][a-zA-Z]+)*$', working_area.get())):
                         if is_valid_phone(agent_phone.get()):
                             if(re.match('^[a-zA-Z]+(?:[\s-][a-zA-Z]+)*$', agent_country.get())):
                              ...
                              Label(screen5, text="Country Name Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                              return                            
                         else:
                             Label(screen5, text="Phone Number Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                             return                              
                     else:
                        Label(screen5, text="Working Area Invaild", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                        return
                 else:
                     Label(screen5, text="Commission Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                     return
             else:
                 Label(screen5, text="Please enter valid name", fg="red",font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                 return
 else:
                                                 ...
                                                 if (amount_validation(customer_outstanding_amount.get())):
                                                     if (is_valid_phone(phone_no.get())):
                                                         if agent_code_validation(agent_code.get()):
                                                             ...
                                                             Label(screen14, text="Agent Code Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                                             return                            
                                                     else:
                                                         Label(screen14, text="Phone Number Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                                         return                              
                                                 else:
                                                     Label(screen14, text="Outstanding Amount Invaild", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                                     return
                                             else:
                                                 Label(screen14, text="Payment Amount Invaild", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                                 return
                                         else:
                                            Label(screen14, text="Receive Amount Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                            return
                                     else:
                                         Label(screen14, text="Opening Amount Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                         return
                                 else:
                                     Label(screen14, text="Grade Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                     return
                             else:
                                 Label(screen14, text="Customer Country Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                 return
                     else:
                         Label(screen14, text="Working Area Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                         return
                                       
                 else:
                     Label(screen14, text="Customer City Invalid", fg="red",font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                     return
             else:
                 Label(screen14, text="Customer Name Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                 return
    else:
                                 ...
                                 if(re.match('^[a-zA-Z]+(?:[\s-][a-zA-Z]+)*$', order_description.get())):
                                     if (agent_code_validation(order_agent_code.get())):
                                        ...
                                        # Button(screen16, text='Back ->', width=20, font=("Open Sans", 9,'bold'), bg='brown', fg='white',command=screen16.destroy).place(x=170, y=565) # button to navigate back to login page
                                         ...
                                         Label(screen16, text="Agent Code Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                         return
                                 else:
                                     Label(screen16, text="Order Description Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                     return                            
                             else:
                                 Label(screen16, text="Order Date Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                                 return                              
                         else:
                             Label(screen16, text="Advance Amount Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                             return
                             
                     else:
                        Label(screen16, text="Advance Amount Invaild", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                        return
                 else:
                     Label(screen16, text="Order Amount Invalid", fg="red", font=("calibri", 11), width='30', anchor=W, bg='white').place(x=0, y=570)
                     return
         else:
...
def add_customer():
    ...
    connection = pymysql.connect(host="localhost", user="root",passwd="", database="project") # database connection
    cursor = connection.cursor()
    select_query = "SELECT AGENT_CODE FROM agents  "
    cursor.execute(select_query)
    agent_codes = cursor.fetchall()
    connection.commit() # commiting the connection then closing it.
    connection.close()
    agent=list(agent_codes)
    li=[]
    ...
    Combobox(screen14, width=9, values = li ,textvariable = agent_code).place(x=670,y=480)
...
def add_orders():
   ...
   # order_no=StringVar()
    ...
    year_choices= list(range(2007,2021))
    ...
    Combobox(screen16, width=9, values = li ,textvariable = agent_code).place(x=650,y=410)
...
def welcome_page():
 ...
 photo2 = PhotoImage(file="C:\\Users\\aksha\\Desktop\\PYTHON\\Classes\\Project\\BWimage2.png") # opening right side image - Note: If
 Label2 = Label(screen2, image=photo2, text="") # attaching image to the label
 Label2.place(x=0, y=73)
 Label2.image = photo2
 Button(screen2, text='ADD & DISPLAY DATA', width=45, font=("Roboto", 13, 'bold'),bg='#E0ACD5', fg='black',command=tables).place(x=500, y=220)
         ...
         if tnc.get(): # checking for acceptance of agreement
             if re.match("^.+@(\[?)[a-zA-Z0-9-.]+.([a-zA-Z]{2,3}|[0-9]{1,3})(]?)$", email.get()):
                 ...
                 if password.get() == repassword.get(): # checking both password match or not
                     ...
                     connection.commit() # commiting the connection then closing it.
                     connection.close() # closing the connection of the database
                     Label(screen38, text="Registration Sucess", fg="green", font=("calibri", 13), width='30', anchor=W, bg='#dfe6e9').place(x=0, y=570) # printing successful registration message
                     Button(screen38, text='Proceed to Login ->', width=15, height=1, font=("Roboto",13, 'bold'), bg='#2159ff', fg='black',command=screen38.destroy).place(x=600, y=630) # button to navigate back to login page
                 else:
...
def main_screen():
 ...
 photo1 = PhotoImage(file="C:\\Users\\aksha\\Desktop\\PYTHON\\Classes\\Project\\BWimage2.png") # opening right side image - Note: If C:\Users\aksha\Desktop
